...
describe('createLogger', function () {
    ...
    it('should create a suitable logger with minimal config', function() {
        var s = new Splunk({ splunk: { token: 'foo' }});
        assert.ok(s instanceof Splunk);
    });
    ...
    it('should set the name property', function() {
        var s = new Splunk({ splunk: { token: 'foo' }});
        assert.strictEqual('Splunk', s.name);
    });
    ...
    it('should allow an override for the default host', function() {
        var s = new Splunk({ splunk: { host: 'bar', token: 'foo' }});
    ...
    it('should allow an override for the default host', function() {
        var s = new Splunk({ splunk: { port: 2000, token: 'foo' }});
    ...
    it('should allow an override for the default eventFormatter', function() {
        var s = new Splunk({ splunk: { eventFormatter: 'foo', token: 'foo' }});
